...
> **注意：** `extension/config/` 管理着标准的 B 站关注分类（字符串数组）。
> - **优先加载用户配置**：扩展和 CLI 都会优先尝试读取 `extension/config/follow-categories.json`。
> - **默认安全回退**：如果用户配置缺失，系统会自动回退读取 `extension/config/follow-categories.default.json`。
> - 这份分类名为：在 **CLI** 中它是关注分类写入时的唯一推荐集合；在 **Chrome 扩展** 中它被交给大模型作为**自动新建**收藏夹和关注分组时的约束依据（如“知识”、“科技数码”等），以保证新建的名字不杂乱。
...
extension/
├── config/
│   ├── follow-categories.default.json # CLI 与扩展共用的默认关注分类配置
│   └── follow-categories.json         # (受 Git 忽略) 本地自定义分类配置
